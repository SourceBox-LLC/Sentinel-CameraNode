...
| POST | `/api/auth/login` | Body `{password}` — argon2-verifies against `AuthConfig::password_hash`, sets a signed `HttpOnly`/`SameSite=Strict` session cookie on success. Never behind the guard below (it's how you get a session in the first place). |
...
- `bind = 127.0.0.1` (Connected default): localhost only, no session required. Anyone with shell access could already wipe `data/node.db` directly, so the additional surface is zero.
- `bind = 0.0.0.0` (Local mode, always — or Connected mode with `--lan-streaming`): any LAN device could otherwise reach live HLS/snapshots/recordings, so a local-admin password is mandatory whenever this bind is chosen (enforced at setup time — see `setup::tui`'s Local-mode branch and `setup::run_quick_setup`'s `--lan-streaming` validation, both of which call `server::auth::hash_password`). `server::auth::guard()` — a stateless, HMAC-signed session cookie, not a JWT library or server-side session table — protects `/hls/*` and `/api/*` other than `/api/auth/*` (see `server::http`'s route composition and `server::auth`'s module doc for the full design rationale). **Still don't expose this server to the public internet** — it's LAN-appropriate auth, not a perimeter (no rate limiting, no 2FA). See `docs/runbooks/local-mode-setup.md` for the operator-facing threat model.
